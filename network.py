...
def get_graph(x, is_training):
    ...
    conv2 = tf.layers.conv2d(inputs=pool1, filters=32, kernel_size=[3, 3], strides=1, padding='SAME', name='conv2',kernel_initializer=tf.contrib.layers.xavier_initializer())
    ...
    conv3 = tf.layers.conv2d(inputs=pool2, filters=32, kernel_size=[3, 3], strides=1, padding='SAME', name='conv3',kernel_initializer=tf.contrib.layers.xavier_initializer())
...
#    bn4 = tf.layers.batch_normalization(pool4, training=is_training, name='bn4')
    ...
    flat = tf.reshape(pool3, [-1, fc1_input_count], name='relu4_flat')
...
#    bn6 = tf.layers.batch_normalization(fc2, training=is_training, name='bn6')
    fc3 = tf.layers.dense(inputs=fc2, units = 6, name='fc3',kernel_initializer=tf.contrib.layers.xavier_initializer())
